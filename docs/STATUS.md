...
- Intensidad del preset (REVISADA tras aclaración del usuario a mitad de sesión — ver `decisions.md`): el slider ya NO interpola los sliders individuales. Ahora es un fundido de imagen completa al final del pipeline en GPU (`lookIntensity.wgsl`, nuevo `CompositePass`): mezcla por píxel el resultado ya revelado contra la foto original tal cual se cargó (0% = original exacto, 100% = look completo). Deshabilitado hasta que se importa un preset. Requirió dos métodos nuevos en `Pipeline` (`renderFinalToTexture`, `displayFinalWithTextures`) y una textura intermedia nueva por imagen (`encodedTexture`).
- Verificado en el navegador con la imagen de referencia real (`docs/reference/portra400-grain-reference.jpg`): grano visiblemente más fino en el mínimo del slider, export/import de preset con contenido y nombre de archivo correctos. La mezcla de intensidad se verificó leyendo el píxel central del canvas a 0/50/100% con un preset extremo: a 0% coincide exacto con el original, a 100% con el resultado completo, a 50% es la media exacta — confirma el lerp de imagen. Se encontró y arregló un bug real durante la primera verificación (ver abajo). `tsc --noEmit`, `vitest run` (23/23) y `vite build` sin errores.

## Bug encontrado y arreglado esta sesión
Al aplicar un preset, las etiquetas de valor con dos decimales (grano, tamaño de grano, acutancia, suavizado, halation, exposición) se leían de vuelta desde `slider.value` después de asignarle un string tipo `"1.00"` — los `<input type="range">` normalizan ese valor internamente y lo devuelven como `"1"`, así que la etiqueta perdía el formato (mostraba "Halation: 1×" en vez de "1.00×"). Arreglado usando directamente el string ya formateado para la etiqueta, sin releerlo del slider. Corregido antes de que el usuario aclarara el mecanismo de intensidad, así que sigue vigente con la implementación nueva.
...
5. Confirmar con el usuario si "Intensidad del preset" debería funcionar también sin haber importado un preset (fundido general del look actual vs. la foto) — el mecanismo ya lo soporta, solo falta quitar el `disabled` inicial si lo piden.
...
- El preview en tiempo real ahora hace un pass GPU extra por frame (el fundido de intensidad, pantalla completa, incluso a 100%) — no se ha medido impacto en rendimiento, debería ser despreciable pero revisar si se nota lag en imágenes muy grandes.
- "Intensidad del preset" solo está disponible tras importar un preset — ver "Próxima acción" punto 5 si el usuario quiere que sea un control general.
